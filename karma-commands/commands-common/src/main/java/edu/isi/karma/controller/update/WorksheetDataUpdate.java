...
/**
 * Update the data of the Worksheet
 *
 */
public class WorksheetDataUpdate extends AbstractUpdate {
	...
	private enum JsonKeys {
		...
		nodeId, rowId, isSelected, rowValueArray, isConceptNode
	}
			...
			for (VHNode vNode : orderedHnodeIds) {
				if(vNode.isVisible()) {
					...
					if (vNode.hasNestedTable()) {
						...
						/**
						 * for {@link ConceptCellValue} the {@link ConceptCellValue#asString()} method only return
						 * selected concept, so in order to display all the concept we use
						 * {@link ConceptCellValue#getConceptsAsString()} method instead.
						 *
						 * @author Danish
						 */
						String nodeVal;
						if(rowNode.getValue() instanceof ConceptCellValue){
							nodeVal = ((ConceptCellValue) rowNode.getValue()).getConceptsAsString();
							nodeObj.put(JsonKeys.isConceptNode.name(), true);
						}
						else{ // for all other CellValue implementations.
							nodeVal = rowNode.getValue().asString();
							nodeObj.put(JsonKeys.isConceptNode.name(), false);
						}
