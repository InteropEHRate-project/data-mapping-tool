	...
	function PrivateConstructor() {
		...
		function init() {
			...
			$('#btnSaveFormat', dialog).on('click', function(e) {
				e.preventDefault();
				if(dialog.data("isCatalogResource")){
					saveDialogCatalogResource(e);
				}
				else{
					saveDialog(e);
				}
		...
		function saveDialogCatalogResource(e) {
			//TODO: in order to support different file types, following classes can be modified to look for downloaded files in the catalog directory.
			// ImportCSVFileCommandFactory: Done
			// ImportAvroFileCommandFactory: Done
			// ImportExcelFileCommandFactory: Done
			// ImportJSONFileCommandFactory: Done
			// ImportJSONLinesFileCommandFactory: Done
			// ImportOntologyCommandFactory: Done
			// ImportXMLFileCommandFactory: Done
			// KMLFileTransferHandler:
			// ApplyHistoryFromR2RMLModelCommandFactory:

			console.log("saveDialogCatalogResource Clicked");

			let fileName = dialog.data("fileName");
			let datasetName = dialog.data("datasetName");

			console.log("Selected file: " + datasetName+"/"+fileName);
			...
			urlString += "&useDownloadedFile=true";
			urlString += "&fileName="+fileName;
			urlString += "&datasetName="+datasetName;
			...
			$.ajax({
				...
				complete: function(xhr, textStatus) {
					...
					if (RequireFilter)
						SelectColumnsDialog.getInstance(json, selectedFormat).show();
					else {
						$("#fileOptionsDialog").data("isFilterSelected", $("input:checkbox[name='FilterCheck']", dialog).prop('checked'));
						FileOptionsDialog.getInstance().show(json, selectedFormat, "", false);
					}
				},
				error: function(xhr, textStatus) {
					alert("Error occurred with Import" + selectedFormat + "Command");
				}
			});
		}
		...
		function show(data) {
			...
			dialog.data("isCatalogResource", false);
		...
		function showForCatalogResource(data) {
			let fileName = data["fileName"];
			let datasetName = data["datasetName"];
			dialog.data("fileName", fileName);
			dialog.data("isCatalogResource", true);
			dialog.data("datasetName", datasetName);
			dialog.modal({
				keyboard: true,
				show: true,
				backdrop: 'static'
		...
		return { //Return back the public methods
			show: show,
			showForCatalogResource: showForCatalogResource,
			init: init
		};
	};
