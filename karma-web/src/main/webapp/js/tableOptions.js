...
var FetchModelDialog = (function() {
	...
	function PrivateConstructor() {
		...
		function init() {
			...
			dialog.on('show.bs.modal', function(e) {
				hideError();
				$('#txtR2RML_URL_fetch').val(backend_karma_model);
			});
...
var ExportCSVModelDialog = (function() {
	...
	function PrivateConstructor(mode) {
		...
		function initCSVDataDialog() {
			$("input#csvDataEndPoint").val(backend_karma_data);
			fetchGraphsFromTripleStore(backend_karma_data, $('#csvDataGraphList'));
...
/**
 * function to initialize publish EML dialog. Generate EML from either
 * existing RDF or generate a new RDF before conversion.
 *
 */
var PublishEMLDialog = (function() {
	var instance = null;

	function PrivateConstructor() {
		var dialog = $("#publishEMLDialog");
		var publishRdfDialog = $("#publishRDFDialog");
		var emlConversionService_URL = $('#emlConversionService_URL', dialog);
		var worksheetId;
        ...
        function init() {
			...
			dialog.on('show.bs.modal', function(e) {
				...
				// text box url for Conversion service
				if (!emlConversionService_URL.val()) {
					emlConversionService_URL.val(eml_service_url);
				}
			});

			//Initialize handler for New Rdf button
			$('#newRdf', dialog).on('click', function(e) {
				e.preventDefault();
				hide();
				PublishRDFDialog.getInstance().show(worksheetId);
				dialog.data("publishEML",true);
			});

			//Initialize handler for Existing Rdf button
			$('#oldRdf', dialog).on('click', function(e) {
				e.preventDefault();
				publishEML();
				hide();
			});
		...
		function publishEML() {
			var info = generateInfoObject(worksheetId, "", "PublishEMLCommand");
			info["emlServiceURL"] = $("#emlConversionService_URL", dialog).val();
			returnFunc(info);
		}
