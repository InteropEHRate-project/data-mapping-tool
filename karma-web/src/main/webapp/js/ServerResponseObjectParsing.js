...
function parse(data) {
	...
	$.each(data["elements"], function(i, element) {
		...
		if (element["updateType"] == "WorksheetListUpdate") {
			...
			$.sticky("Worksheet or Command deleted");
		...
		} else if (element["updateType"] == "PublishEMLUpdate") {
			var titleDiv = $("div#" + element["worksheetId"] + " div.WorksheetTitleDiv");
			// Remove existing link if any
			$("a.EMLDownloadLink", titleDiv).remove();
			$("a.EMLContextDownloadLink", titleDiv).remove();
			var downloadLink = $("<a>").attr("href", element["fileUrl"]).text("EML").addClass("EMLDownloadLink DownloadLink").attr("target", "_blank");
			$("div#WorksheetOptionsDiv", titleDiv).after(downloadLink);
			if (element["contextUrl"] != undefined) {
				var contextDownloadLink = $("<a>").attr("href", element["contextUrl"]).text("Context").addClass("EMLContextDownloadLink DownloadLink").attr("target", "_blank");
				$("div#WorksheetOptionsDiv", titleDiv).after(contextDownloadLink);
			}
...
/**
 * create the nodes and attach additional html to perforce the modification for each node
 *
 * @param worksheetId
 * @param rows
 * @param dataTable
...
function addWorksheetDataRecurse(worksheetId, rows, dataTable, isOdd) {
	...
	$.each(rows, function(index, rowWithMetaData) {
		...
		$.each(row, function(index2, cell) {
			...
			/**
			 * if the node is a Concept we add a radio group button
			 * and in case of change event happens meaning setting different concept,
			 * it modify the value of the Node.
			 *
			 * @author Danish
			 * @date: 2019-07-24
			 */
			else if (cell["isConceptNode"]){
				var json = $.parseJSON(cell["expandedValue"]);
				var dataDiv3 = $("<div>").addClass("wk-value")
									.css({"overflow-y": "scroll",
										"max-height":"80px"
									});
				dataDiv.addClass(cell["columnClass"]);
				if($.isArray(json) && json.length){ // if array is null we leave the cell empty
					var divGroupBtnTag = $("<div>").addClass("btn-group-vertical")
							...
							.data("concepts",json);
					/**
					 * loop over list of meanings for each row and creating list of buttons
					 */
					$.each(json,function(index3, obj){
						var meaning = obj["meaning"];
						var name,description,lang;
						//this is a hack to get the name of the concept
						for(var locale in meaning["name"]["strings"]){
							var string = meaning["name"]["strings"][locale][0];
							name = string.charAt(0).toUpperCase() + string.slice(1);
							lang = locale;
							break;
						}
						//this is a hack to get the gloss of the concept
						for(var locale in meaning["description"]["strings"]){
							description = meaning["description"]["strings"][locale][0];
							break;
						}

						/**
						 * attach popover to each button
						 */
						var labelTag = $("<lable>")
							.text(meaning["id"] +"-"+ name)
							.addClass((obj["isSelected"]) ? "btn btn-primary change-trigger active" : "btn btn-primary change-trigger")
							.data("concepts",json)
							.popover({
								placement : 'right',
								title : meaning["id"] +" "+ name,
								trigger : 'manual',
								html : true,
								container: 'body',
								content : function(){ //popover content

									/**
									 * clickable url for more detail of the meaning
									 */
									var externalLink = $("<a>")
										.html(" More Detail");
									/**
									 * Select button to change the selected meaning
									 */
									...
									var cancelButton = $("<button>")
										.addClass("btn btn-link cancel")
										.attr("type","button")
										.html("Cancel");
									var buttons = $("<div>")
										.addClass("clearfix col-sm-10")
									...
									return $("<div>").append(content).append(buttons);
								}
							})
							...
							.on('shown.bs.popover', function (eventShown) {
								...
								var json = $(this).data("concepts");

								/**
								 * handler if More Detail button is clicked for a popover
								 */
								$popup.find('a').click(function (e){
									e.preventDefault();
									$popup.popover('hide');
									window.open(kos_service_url + "#/kb/formal/" + meaning["id"] + "/" + lang, "_blank");
								});

								/**
								 * handler if cancel button is clicked for a popover
								 */
								...
								/**
								 * handler if select button is clicked for a popover
								 */
								$popup.find('button.select').click(function (e) {
									e.preventDefault();
									$popup.popover('hide');
									...
									var conceptCellValue = {};
									// serializing it properly for ConceptCellValue
									conceptCellValue["type"] = "ConceptCellValue";
									conceptCellValue["concepts"] = arr;
									console.log("Set new value:" + JSON.stringify(conceptCellValue));
									submitTableCellEdit(worksheetId, cell["nodeId"], JSON.stringify(conceptCellValue));
								});
							})
						...
						;
						divGroupBtnTag.append(labelTag);
					});
					dataDiv3.append(divGroupBtnTag);
						...
						if (!dataDiv3.hasClass("editable")) {
							dataDiv3.editable({
								...
								success: function(response, newValue) {
									...
									var stringCellValue = {};
									stringCellValue["type"] = "StringCellValue";
									stringCellValue["value"] = newValue;
									submitTableCellEdit(worksheetId, cell["nodeId"], JSON.stringify(stringCellValue));
								},
								showbuttons: 'bottom',
								mode: 'popup',
...
/**
 * modifying or editing the value of a node.
 * Java Polymorphism is used with Gson to DeSerialize the
 * the json to StringCellValue or ConceptCellValue Object. for more detail check the
 * edu.isi.karma.controller.command.worksheet.EditCellCommandFactory.java
 *
 * Modified by: Danish 25 July 2019
 *
 * @param worksheetId
 * @param nodeId
 * @param value
